...
-- Load Helpers and Base Logic first so they are available to other scripts
...
spyro_3_year_of_the_dragon_location = {}
spyro_3_year_of_the_dragon_location.__index = spyro_3_year_of_the_dragon_location

-- Now you can process sunriseSpringData for creating checkboxes, spawning gems, etc.
...
function spyro_3_year_of_the_dragon_location.new(name)
    local self = setmetatable({}, spyro_3_year_of_the_dragon_location)
    if name then
...
function spyro_3_year_of_the_dragon_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = spyro_3_year_of_the_dragon_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function spyro_3_year_of_the_dragon_location:connect_two_ways(exit, rule)
...
function spyro_3_year_of_the_dragon_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function spyro_3_year_of_the_dragon_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function spyro_3_year_of_the_dragon_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function spyro_3_year_of_the_dragon_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function spyro_3_year_of_the_dragon_location:accessibility()
...
-- 
function spyro_3_year_of_the_dragon_location:discover(accessibility, keys)
...
-- 1. The Root
entry_point = spyro_3_year_of_the_dragon_location.new("entry_point")

-- logic.lua - The Master Controller
require("scripts/logic/sunrise_spring")
--require("scripts/logic/worlds/midday_gardens")
--require("scripts/logic/worlds/evening_lake")
--require("scripts/logic/worlds/midnight_mountain")
